  ...
  //}else{
    ...
    switch (protocol) {
      ...
      case 1: //CTO
        ...
        var amount = 10.0   //user_input.amount;
        //the address they would like to buy the coin for
    		const pub_key = user_input.publicKey;
      ...
      case 2: //P2P
        ...
        **/
        const public_key = user_input.publicKey;
        const priv_key = user_input.privateKey;
        const subject = user_input.subject;
        const body = user_input.body;
        var amounts = user_input.amount;
        const dest_address = user_input.recipients;
        ...
        /*
        ******************************************************************************************************************
        To verify proof of funds if pay per use model is to be used
        ******************************************************************************************************************
        */
        ...
        const OODLE = 1;//get_OODLE(payer_wallet, amounts);
        ...
        if(OODLE != 0){
          ...
          //structure locking script using interpreter commands and the destination address
      		var output_script = [OP_DUP, OP_HASH160, dest_address[0].trim(), OP_EQUALVERIFY, OP_CHECKSIG];
        ...
        //************************************************************************************************
        const key = crypto.scryptSync(public_key, 'salt', 24); //create key
        ...
        var encrypted = cipher.update(email, 'utf8', 'hex') + cipher.final('hex'); // encrypted text
        ...
        new_Transactions.input(count, input_script);
      	new_Transactions.output(amounts, output_script, encrypted, iv.toString('hex'));
      ...
      //********************************************************************************************************************
      //
      //Structure transaction of sender to verify that it is all valid
      //
      //********************************************************************************************************************
      ...
      //create new transaction object at current time and index
      var previous_Transaction = new transaction(Date.now(), count);

      try{
        //hash transaction
        var hash = previous_Transaction.ripemd160(previous_Transaction.sha256(public_key));
      } catch (e){
      ...
      //creating transaction unlocking script, current empty until its spent
      previous_Transaction.input(count, []);
      //output "locking" script
      previous_Transaction.output(amount, output_script);
      ...
      //********************************************************************************************************************
          ...
          for(var i = 0; i < 1; i++){//funds_F_T.length; i++){
            if(validate_Transaction(new_Transactions, previous_Transaction, 0)){     //funds_F_T[i], 0)){
...
/*
**************************************************************************************
If pay per use model is to be used
*/
      ...
      try{
//console.log(chalk.yellow(wallet_Address));
        //wallet_Address = Buffer.from(wallet_Address, 'hex');
        var wallet = get_User_Wallet_Transactions(wallet_Address);    //ripemd160(sha256(wallet_Address)).toString('hex'));
    ...
    **/
    ...
    if (isMainThread) {
      ...
      if(index == 1){
        ...
        sharedBuffer = new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT * temp_Chain.length * 13);
      }else{
        //same
